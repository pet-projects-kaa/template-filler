...
public sealed class AppDatabase
{
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            command.CommandText = """
                ...
                CREATE TABLE IF NOT EXISTS UploadedFonts (
                    Id TEXT NOT NULL PRIMARY KEY,
                ...
                CREATE INDEX IF NOT EXISTS IX_UploadedFonts_UserId_CreatedAt
                    ON UploadedFonts (UserId, CreatedAt DESC);
                """;
    ...
    public async Task<IReadOnlyList<UploadedFontListItem>> GetUploadedFontsAsync(
        Guid userId,
        CancellationToken cancellationToken)
    {
        await EnsureInitializedAsync(cancellationToken);
        ...
        await using var command = connection.CreateCommand();
        command.CommandText = """
            SELECT Id, Name, OriginalFileName, FileExtension, SizeBytes, CreatedAt
            FROM UploadedFonts
            WHERE UserId = $userId
        ...
        var result = new List<UploadedFontListItem>();
        await using var reader = await command.ExecuteReaderAsync(cancellationToken);
        while (await reader.ReadAsync(cancellationToken))
        {
            result.Add(new UploadedFontListItem(
                Guid.Parse(reader.GetString(0)),
                reader.GetString(1),
                reader.GetString(2),
                reader.GetString(3),
    ...
    public async Task<UploadedFontFile?> GetUploadedFontFileAsync(
        Guid userId,
        Guid fontId,
        CancellationToken cancellationToken)
    {
        ...
        await using var command = connection.CreateCommand();
        command.CommandText = """
            SELECT FontData, ContentType, OriginalFileName, FileExtension
            FROM UploadedFonts
            WHERE Id = $id AND UserId = $userId;
            """;
        command.Parameters.AddWithValue("$id", fontId.ToString("D"));
        command.Parameters.AddWithValue("$userId", userId.ToString("D"));
    ...
    public async Task<UploadedFontListItem> CreateUploadedFontAsync(
        Guid userId,
        string name,
        ...
        CancellationToken cancellationToken)
    {
        ...
        await using var command = connection.CreateCommand();
        command.CommandText = """
            INSERT INTO UploadedFonts
                (Id, UserId, Name, OriginalFileName, ContentType, FileExtension,
                 FontData, SizeBytes, CreatedAt)
            VALUES
                ($id, $userId, $name, $originalFileName, $contentType, $fileExtension,
                 $fontData, $sizeBytes, $createdAt);
            """;
        ...
        command.Parameters.Add("$fontData", SqliteType.Blob).Value = data;
        command.Parameters.AddWithValue("$sizeBytes", data.LongLength);
        command.Parameters.AddWithValue("$createdAt", FormatDate(createdAt));
        await command.ExecuteNonQueryAsync(cancellationToken);

        return new UploadedFontListItem(
            id,
            name,
            originalFileName,
            fileExtension,
    ...
    public async Task<bool> DeleteUploadedFontAsync(
        Guid userId,
        Guid fontId,
        CancellationToken cancellationToken)
    {
        ...
        await using var connection = await OpenConnectionAsync(cancellationToken);
        await using var command = connection.CreateCommand();
        command.CommandText = "DELETE FROM UploadedFonts WHERE Id = $id AND UserId = $userId;";
        command.Parameters.AddWithValue("$id", fontId.ToString("D"));
        command.Parameters.AddWithValue("$userId", userId.ToString("D"));
        return await command.ExecuteNonQueryAsync(cancellationToken) > 0;
    }
